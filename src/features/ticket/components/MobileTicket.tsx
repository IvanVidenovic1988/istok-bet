...
import { TicketItem, closeTicket, removeFromTicket } from '../redux/ticket';
    ...
    return (
        <>
            {isTicketOpen && <div className='lg:mr-3 bg-[#1a1c1d]'>
                <div className='h-[46px] bg-[#ffbb1a] flex-center'>
                    <button
                        onClick={() => dispatch(closeTicket())}
                        className='absolute left-[15px] text-[#2c2e30] font-bold'
                    >X</button>
                ...
                <div className='h-[52px] mx-[10px] flex items-center'>
                    ...
                    <div className='w-[40%] text-[#ffbb1a] text-[14px] flex-center'>
                        Sistem
                    </div>
                </div>
                {isEmpty(ticketItems) ?
                                        ...
                                        {tekme.map((tekma) => (
                                            <div key={tekma.id + (Math.random() * 1000)}>
                                                <div className='mb-2 text-[14px] flex justify-between'>
                                                    <span className='text-[#acacad] lowercase'>{tekma.marketsName} {tekma.marketsSpecialValue.valueOf() > 0 || tekma.marketsSpecialValue.valueOf() < 0 ? <span>( {tekma.marketsSpecialValue} )</span> : <span></span>}</span>
                                                    <button
